...
fn parse_command(text: &str, call_id: &str, commands: &[CommandSpec]) -> Option<CommandCall> {
    ...
    let platform_name = head.split_once('@').map_or(head, |(cmd, _)| cmd);
...
fn extract_attachments(message: &teloxide::types::Message) -> Vec<Attachment> {
    ...
    if let Some(doc) = message.document() {
        atts.push(Attachment {
            mime: doc.mime_type.as_ref().map_or_else(
    ...
    if let Some(voice) = message.voice() {
        atts.push(Attachment {
            mime: voice
                .mime_type
                .as_ref()
                .map_or_else(|| "audio/ogg".into(), std::string::ToString::to_string),
            name: None,
    ...
    if let Some(audio) = message.audio() {
        atts.push(Attachment {
            mime: audio
                .mime_type
                .as_ref()
                .map_or_else(|| "audio/mpeg".into(), std::string::ToString::to_string),
    ...
    if let Some(video) = message.video() {
        atts.push(Attachment {
            mime: video
                .mime_type
                .as_ref()
                .map_or_else(|| "video/mp4".into(), std::string::ToString::to_string),
