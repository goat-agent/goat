...
pub const ID: IntegrationId = IntegrationId::from_static("gdrive");

const SUMMARY: &str = "search and read Drive files";
pub const PREFIX: &str = "gdrive_";

const MCP_URL: &str = "https://drivemcp.googleapis.com/mcp/v1";
const ENV_VAR: &str = "GOAT_GDRIVE_TOKEN";

const SETUP: &str = "connects to Google's hosted Google Drive MCP server, which is in the Workspace Developer Preview and does not register clients dynamically.\n\
     in a Google Cloud project, enable drivemcp.googleapis.com, configure the OAuth consent screen, and create an OAuth client; `goat integration add gdrive` then asks for its id and secret before opening the browser.\n\
     this integration adds `gdrive_*` tools — it does not watch Google Drive or brief you on its own.\n\
     to run headless, set GOAT_GDRIVE_TOKEN.\n\
...
pub fn service() -> McpService {
    McpService::new("gdrive", "Google Drive", ServiceUrl::Fixed(MCP_URL), SETUP)
        .summary(SUMMARY)
        .oauth(SCOPES)
        .preregistered()
        .env_var(ENV_VAR)
        .tools(ToolPolicy::all(PREFIX).deny(DENY))
        .truncation_hint("narrow the search query or request fewer files and call again")
}

#[derive(Debug, Default, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct GdriveBinding {}

fn validate_config(config: &Value) -> IntegrationResult<()> {
    goat_integration_mcp::validate_binding::<GdriveBinding>("gdrive", config)
}
...
mod tests {
    ...
    fn factory_is_registered_in_inventory() {
        ...
        assert!(registry.contains_key("gdrive"));
        assert!(goat_integration::factory_for("gdrive").is_some());
    }

    #[test]
    fn the_service_asks_for_an_oauth_client_of_its_own() {
        let meta = service().build().metadata();
        assert_eq!(meta.id, "gdrive");
        assert_eq!(meta.display, "Google Drive");
