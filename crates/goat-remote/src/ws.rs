...
pub type FrameSink<T> = Pin<Box<dyn Sink<T, Error = WireError> + Send>>;
pub type FrameStream<T> = Pin<Box<dyn Stream<Item = Result<T, WireError>> + Send>>;

pub fn adapt<S, Out, In>(ws: WebSocketStream<S>) -> (FrameSink<Out>, FrameStream<In>)
where
