...
pub const ID: IntegrationId = IntegrationId::from_static("supabase");

const SUMMARY: &str = "inspect projects and run SQL";
pub const PREFIX: &str = "supabase_";

const MCP_URL: &str = "https://mcp.supabase.com/mcp";
const ENV_VAR: &str = "GOAT_SUPABASE_TOKEN";

const SETUP: &str = "connects to Supabase's hosted MCP server; a browser window will ask you to approve access.\n\
     this integration adds `supabase_*` tools — it does not watch Supabase or brief you on its own.\n\
     to run headless, or to recover if the browser flow fails, set GOAT_SUPABASE_TOKEN.\n\
...
pub fn service() -> McpService {
    McpService::new("supabase", "Supabase", ServiceUrl::Fixed(MCP_URL), SETUP)
        .summary(SUMMARY)
        .env_var(ENV_VAR)
        .tools(ToolPolicy::all(PREFIX).deny(DENY))
        .truncation_hint("add a LIMIT, select fewer columns, or narrow the query and call again")
}

#[derive(Debug, Default, Deserialize)]
#[serde(deny_unknown_fields)]
pub(crate) struct SupabaseBinding {}

fn validate_config(config: &Value) -> IntegrationResult<()> {
    goat_integration_mcp::validate_binding::<SupabaseBinding>("supabase", config)
}
...
mod tests {
    ...
    fn factory_is_registered_in_inventory() {
        ...
        assert!(registry.contains_key("supabase"));
        assert!(goat_integration::factory_for("supabase").is_some());
    }

    #[test]
    fn metadata_says_this_integration_only_brings_tools() {
        let meta = service().build().metadata();
        assert_eq!(meta.id, "supabase");
        assert_eq!(meta.display, "Supabase");
