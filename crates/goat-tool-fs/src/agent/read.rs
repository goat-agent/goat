...
use crate::agent::common::{self, DEFAULT_READ_LINES};
...
impl Tool for ReadTool {
    ...
    fn description(&self) -> Cow<'static, str> {
        "Read a UTF-8 text file. Use before edit/write when modifying an existing file; partial reads do not authorize edits.".into()
    }
    ...
    fn call<'a>(&'a self, call: &'a ToolCall, ctx: ToolContext<'a>) -> ToolFuture<'a> {
        Box::pin(async move {
            let args = match serde_json::from_value::<ReadArgs>(call.arguments.clone()) {
                Ok(args) => args,
                Err(e) => return Ok(common::error(format!("invalid read input: {e}"))),
            };
