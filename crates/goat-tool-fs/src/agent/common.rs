...
use goat_tool::{ToolContext, ToolErrorClass, ToolOutput, ToolReadSnapshot};
...
pub fn existing_path(ctx: &ToolContext<'_>, path: &Path) -> Result<PathBuf, String> {
    let resolved = resolve(ctx, path)?;
    fs::canonicalize(&resolved).map_err(|e| format!("invalid path {}: {e}", resolved.display()))
}

pub fn writable_path(ctx: &ToolContext<'_>, path: &Path) -> Result<PathBuf, String> {
    resolve(ctx, path)
}

fn resolve(ctx: &ToolContext<'_>, path: &Path) -> Result<PathBuf, String> {
    ctx.resolve(path).map_err(|e| {
        if e.class() == ToolErrorClass::Policy {
            format!("path escapes agent tool root: {}", path.display())
        } else {
            e.to_string()
        }
    })
...
pub fn store_snapshot(
    ...
    let agent = ctx.agent_context().map_err(|e| e.to_string())?;
    let mut state = agent
        .read_state
        .lock()
