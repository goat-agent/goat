...
use goat_tool_shell::agent::deny_reason;
...
impl Tool for PtyTool {
    ...
    fn call<'a>(&'a self, call: &'a ToolCall, _ctx: ToolContext<'a>) -> ToolFuture<'a> {
        Box::pin(async move {
            let args: PtyArgs = match serde_json::from_value(call.arguments.clone()) {
                Ok(a) => a,
            ...
            Ok(match args.action {
                Action::Open => {
                    ...
                    if !cmd.trim().is_empty()
                        && let Some(reason) = deny_reason(cmd)
                    {
                ...
                Action::Input => {
                    let Some(id) = args.session_id.as_deref() else {
                        return Ok(ToolOutput::error("input requires session_id"));
                    };
                    let empty = vec![];
                ...
                Action::Read => {
                    let Some(id) = args.session_id.as_deref() else {
                        return Ok(ToolOutput::error("read requires session_id"));
                    };
                ...
                Action::List => {
                    ...
                    let sessions: Vec<_> = infos
                        ...
                        .collect();
                    let mut output = ToolOutput::text(text);
                    output.structured_content = Some(json!({ "sessions": sessions }));
                    output
                }

                Action::Close => {
                    let Some(id) = args.session_id.as_deref() else {
                        return Ok(ToolOutput::error("close requires session_id"));
                    };
                ...
                Action::Resize => {
                    let Some(id) = args.session_id.as_deref() else {
                        return Ok(ToolOutput::error("resize requires session_id"));
                    };
                ...
                Action::Signal => {
                    let Some(id) = args.session_id.as_deref() else {
                        return Ok(ToolOutput::error("signal requires session_id"));
                    };
                    let sig = args.signal.as_deref().unwrap_or("int");
                    match self.manager.signal(id, sig) {
                        Ok(()) => {
                            ToolOutput::text(format!("sent SIG{} to {id}", sig.to_uppercase()))
                        }
                        Err(e) => ToolOutput::error(e),
                    }
                }
