...
use goat_tool::{
    AgentContext, Tool, ToolCall, ToolContext, ToolFuture, ToolName, ToolOutput, ToolRegistry,
};
...
pub fn register(registry: &mut ToolRegistry, store: Arc<dyn Store>) {
    registry.insert(Arc::new(GoalTool { store }));
}
...
impl GoalTool {
    async fn run(&self, ac: &AgentContext, cmd: GoalCmd) -> ToolOutput {
        match cmd {
            GoalCmd::Create {
                title,
                detail,
                ...
                let new = NewGoal {
                    ...
                    origin_conv: Some(ac.conversation.clone()),
            ...
            GoalCmd::List => match self.store.active_goals(ac.id).await {
                Ok(goals) => {
                    let out: Vec<_> = goals
                        .into_iter()
                        .map(|g| {
...
impl Tool for GoalTool {
    ...
    fn call<'a>(&'a self, call: &'a ToolCall, ctx: ToolContext<'a>) -> ToolFuture<'a> {
        Box::pin(async move {
            ...
            let cmd: GoalCmd = match serde_json::from_value(call.arguments.clone()) {
                Ok(c) => c,
                Err(e) => return Ok(ToolOutput::error(format!("invalid goal command: {e}"))),
            };
            Ok(self.run(ac, cmd).await)
        })
    }
}
...
mod tests {
    ...
    async fn setup() -> (Arc<dyn Store>, AgentContext) {
