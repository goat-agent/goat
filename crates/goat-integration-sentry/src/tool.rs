...
pub async fn register(
    ...
    for tool in tools {
        let Ok(name) = ToolName::new(tool_name(&tool.name)) else {
            warn!(tool = %tool.name, "skipping sentry mcp tool with unusable name");
            continue;
        };
        ...
        Ok(tools) => {
            if let Ok(raw) = serde_json::to_string(&tools)
                && let Err(e) = runtime
                    ...
                    .await
            {
                warn!(error = %e, "failed to cache sentry tool list");
            }
            tools
        }
        Err(e @ (IntegrationError::Auth(_) | IntegrationError::Config(_))) => {
            warn!(error = %e, "sentry tool discovery was rejected; registering nothing");
            Vec::new()
        }
        Err(e) => {
            warn!(error = %e, "sentry tool discovery failed; using cached list");
            match runtime
    ...
    async fn call(&self, ctx: ToolContext, call: ToolCall) -> ToolOutput {
        let Some(binding) = self.bindings.get(&ctx.persona) else {
            return ToolOutput::error(
                "sentry is not configured for this agent; run `goat agent integration add sentry`",
            );
        };
        let client_id = client_id_of(binding);
        let auth =
...
mod tests {
    ...
    fn vendor_prefix_is_added_once_and_never_doubled() {
        assert_eq!(tool_name("search_issues"), "sentry_search_issues");
        assert_eq!(tool_name("sentry_search_issues"), "sentry_search_issues");
        assert!(ToolName::new(tool_name("get_issue_details")).is_ok());
    ...
    fn cached_tools_round_trip() {
        let cached = vec![CachedTool {
            ...
            description: "Search for grouped issues".into(),
            input_schema: json!({ "type": "object" }),
        }];
        let raw = serde_json::to_string(&cached).unwrap();
        let back: Vec<CachedTool> = serde_json::from_str(&raw).unwrap();
        assert_eq!(back[0].name, "search_issues");
        assert_eq!(tool_name(&back[0].name), "sentry_search_issues");
    }
