...
pub async fn register(
    ...
    for tool in tools {
        let Ok(name) = ToolName::new(tool_name(&tool.name)) else {
            warn!(tool = %tool.name, "skipping slack mcp tool with unusable name");
            continue;
        };
        ...
        Ok(tools) => {
            if let Ok(raw) = serde_json::to_string(&tools)
                && let Err(e) = runtime
                    ...
                    .await
            {
                warn!(error = %e, "failed to cache slack tool list");
            }
            tools
        }
        Err(e) => {
            warn!(error = %e, "slack tool discovery failed; using cached list");
            match runtime
...
async fn fetch_live(
    ...
    let token = mcp::resolve_auth(credentials, &binding.account)?;
    ...
    async fn call(&self, ctx: ToolContext, call: ToolCall) -> ToolOutput {
        let Some(binding) = self.bindings.get(&ctx.persona) else {
            return ToolOutput::error(
                "slack is not configured for this agent; run `goat agent integration add slack`",
            );
        };
        let token = match mcp::resolve_auth(&self.credentials, &binding.account) {
            Ok(token) => token,
            Err(e) => return ToolOutput::error(e.to_string()),
        };
        let session = match mcp::connect(&token).await {
            Ok(session) => session,
            Err(e) => return ToolOutput::error(e.to_string()),
        };
...
mod tests {
    ...
    fn vendor_prefix_is_added_once_and_never_doubled() {
        assert_eq!(tool_name("search_public"), "slack_search_public");
        assert_eq!(tool_name("slack_search_public"), "slack_search_public");
        assert!(ToolName::new(tool_name("read_channel")).is_ok());
    ...
    fn cached_tools_round_trip() {
        ...
        assert_eq!(back[0].name, "slack_send_message");
        assert_eq!(tool_name(&back[0].name), "slack_send_message");
    }
