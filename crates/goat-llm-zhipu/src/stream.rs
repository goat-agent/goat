    ...
    let s = async_stream::stream! {
        ...
        while let Some(item) = stream.next().await {
            ...
            let chunk: Chunk = match serde_json::from_str(&raw.data) {
                ...
                Err(e) => {
                    parse_failures += 1;
                    warn!(error = ?e, consecutive = parse_failures, "bad zhipu SSE payload");
...
mod tests {
    ...
    fn parses_chunk_with_reasoning_content() {
        let d = r#"{"choices":[{"delta":{"reasoning_content":"hmm"}}]}"#;
    ...
    fn parses_finish_with_usage() {
        let d = r#"{"choices":[{"delta":{},"finish_reason":"sensitive"}],"usage":{"prompt_tokens":2,"completion_tokens":4}}"#;
        let chunk: Chunk = serde_json::from_str(d).unwrap();
        assert_eq!(chunk.choices[0].finish_reason.as_deref(), Some("sensitive"));
