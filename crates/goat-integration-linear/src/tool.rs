...
pub async fn register(
    ...
    for tool in tools {
        let Ok(name) = ToolName::new(format!("linear_{}", tool.name)) else {
            warn!(tool = %tool.name, "skipping linear mcp tool with unusable name");
            continue;
        };
        ...
        Ok(tools) => {
            if let Ok(raw) = serde_json::to_string(&tools)
                && let Err(e) = runtime
                    ...
                    .await
            {
                warn!(error = %e, "failed to cache linear tool list");
            }
            tools
        }
        Err(e) => {
            warn!(error = %e, "linear tool discovery failed; using cached list");
            match runtime
    ...
    async fn call(&self, ctx: ToolContext, call: ToolCall) -> ToolOutput {
        let Some(binding) = self.bindings.get(&ctx.persona) else {
            return ToolOutput::error(
                "linear is not configured for this agent; run `goat agent integration add linear`",
            );
        };
        let client_id = binding.config.get("client_id").and_then(Value::as_str);
...
mod tests {
    ...
    #[test]
    fn cached_tools_round_trip_and_prefix_validates() {
        ...
        assert_eq!(back[0].name, "list_issues");
        assert!(ToolName::new(format!("linear_{}", back[0].name)).is_ok());
        assert!(ToolName::new("linear_bad.name").is_err());
    }
    ...
    fn first_binding_is_deterministic() {
        ...
        let (first_a, _) = first_binding(&map).unwrap();
