...
pub const MCP_URL: &str = "https://mcp.linear.app/mcp";
pub const ENV_VAR: &str = "LINEAR_API_KEY";
...
) -> IntegrationResult<LinearAuth> {
    let key = CredentialKey::integration("linear", account);
    match credentials.resolve(&key, Some(ENV_VAR)) {
        Some(Credential::ApiKey(secret) | Credential::ApiKeyWithEndpoint { secret, .. }) => {
            Ok(LinearAuth::ApiKey(secret.expose().to_string()))
        }
        Some(Credential::OAuth(tokens)) => {
            let client_id = client_id.ok_or_else(|| {
                IntegrationError::Config(
                    "linear binding missing `client_id`; run `goat agent integration add linear`"
                        .into(),
                )
            })?;
            Ok(LinearAuth::OAuth {
        ...
        None => Err(IntegrationError::Auth(format!(
            "no linear credential for account `{account}`; run `goat agent integration add linear` or set {ENV_VAR}"
        ))),
    }
}
...
pub async fn connect(auth: &LinearAuth) -> IntegrationResult<LinearSession> {
    match auth {
        LinearAuth::ApiKey(key) => {
            let config =
                StreamableHttpClientTransportConfig::with_uri(MCP_URL).auth_header(key.clone());
            let transport =
        ...
        LinearAuth::OAuth { client_id, tokens } => {
            ...
            let OAuthState::Authorized(manager) = oauth else {
                return Err(IntegrationError::Auth(
                    "linear oauth credentials did not authorize".into(),
                ));
            };
    ...
    tokio::time::timeout(START_TIMEOUT, fut)
        ...
        .map_err(|_| {
            IntegrationError::Service(format!(
                "linear mcp initialize timed out after {}s",
                START_TIMEOUT.as_secs()
            ))
        })?
        .map_err(|e| IntegrationError::Service(format!("linear mcp initialize failed: {e}")))
}
...
impl LinearSession {
    pub fn server_name(&self) -> String {
        self.service
            ...
            .map_or_else(|| "linear mcp".to_string(), |server| server.name)
    }
    ...
    pub async fn call(&self, tool: &str, arguments: Value) -> IntegrationResult<Value> {
        ...
        let handle = self
            ...
            .await
            .map_err(|e| IntegrationError::Service(format!("linear mcp request failed: {e}")))?;
        let result = handle
            .await_response()
            .await
            .map_err(|e| IntegrationError::Service(format!("linear mcp request failed: {e}")))?;
        let ServerResult::CallToolResult(result) = result else {
            return Err(IntegrationError::Service(
                "unexpected linear mcp response".into(),
            ));
        };
        if result.is_error.unwrap_or(false) {
            return Err(IntegrationError::Service(format!(
                "linear mcp tool `{tool}` failed: {}",
                result_text(&result)
            )));
        }
    ...
    pub async fn list_tools(&self) -> IntegrationResult<Vec<rmcp::model::Tool>> {
        tokio::time::timeout(CALL_TIMEOUT, self.service.list_all_tools())
            .await
            .map_err(|_| IntegrationError::Service("linear mcp list_tools timed out".into()))?
            .map_err(|e| IntegrationError::Service(format!("linear mcp list_tools failed: {e}")))
    }

    pub async fn close(mut self) {
        if let Err(e) = self.service.close_with_timeout(CLOSE_TIMEOUT).await {
            warn!(error = %e, "failed to close linear mcp session");
        }
    }
}
...
pub async fn persist_tokens(credentials: &CredentialStore, account: &str, session: &LinearSession) {
    ...
    if changed && let Err(e) = credentials.store(&key, Credential::OAuth(fresh)) {
        warn!(error = %e, "failed to persist refreshed linear tokens");
    }
}
...
mod tests {
    ...
    #[test]
    fn resolve_auth_prefers_env_api_key() {
        ...
        store
            .store(
                &CredentialKey::integration("linear", "default"),
        ...
        assert!(matches!(
            ...
            Ok(LinearAuth::OAuth { client_id, .. }) if client_id == "client-1"
        ));
    }
}
