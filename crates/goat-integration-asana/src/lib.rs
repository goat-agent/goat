...
pub const ID: IntegrationId = IntegrationId::from_static("asana");

const SUMMARY: &str = "read and update tasks and projects";
pub const PREFIX: &str = "asana_";

const MCP_URL: &str = "https://mcp.asana.com/sse";
const ENV_VAR: &str = "GOAT_ASANA_TOKEN";

const SETUP: &str = "connects to Asana's hosted MCP server; a browser window will ask you to approve access.\n\
     this integration adds `asana_*` tools — it does not watch Asana or brief you on its own.\n\
     to run headless, or to recover if the browser flow fails, set GOAT_ASANA_TOKEN.\n\
...
pub fn service() -> McpService {
    McpService::new("asana", "Asana", ServiceUrl::Fixed(MCP_URL), SETUP)
        .summary(SUMMARY)
        .env_var(ENV_VAR)
        .tools(ToolPolicy::all(PREFIX).deny(DENY))
        .truncation_hint("narrow the project or assignee, or fetch a single task instead")
}

#[derive(Debug, Default, Deserialize)]
#[serde(deny_unknown_fields)]
pub(crate) struct AsanaBinding {}

fn validate_config(config: &Value) -> IntegrationResult<()> {
    goat_integration_mcp::validate_binding::<AsanaBinding>("asana", config)
}
...
mod tests {
    ...
    fn factory_is_registered_in_inventory() {
        ...
        assert!(registry.contains_key("asana"));
        assert!(goat_integration::factory_for("asana").is_some());
    }

    #[test]
    fn metadata_says_this_integration_only_brings_tools() {
        let meta = service().build().metadata();
        assert_eq!(meta.id, "asana");
        assert_eq!(meta.display, "Asana");
