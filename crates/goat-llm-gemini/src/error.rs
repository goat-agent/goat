...
mod tests {
    ...
    fn parses_gemini_stop_reasons() {
        ...
        assert!(matches!(parse_stop("SAFETY"), StopReason::Refused));
        assert!(matches!(parse_stop("RECITATION"), StopReason::Refused));
        assert!(matches!(parse_stop("BLOCKLIST"), StopReason::Refused));
