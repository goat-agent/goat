...
pub fn register(registry: &mut ToolRegistry, store: Arc<dyn Store>) {
    registry.insert(Arc::new(ObservationTool { store }));
}
...
impl Tool for ObservationTool {
    ...
    fn call<'a>(&'a self, call: &'a ToolCall, ctx: ToolContext<'a>) -> ToolFuture<'a> {
        Box::pin(async move {
            ...
            let args: Args = match serde_json::from_value(call.arguments.clone()) {
                ...
                Err(e) => {
                    return Ok(ToolOutput::error(format!(
                        "invalid observation arguments: {e}"
                    )));
                }
            };

            if let Some(id) = args.id {
                return Ok(match self.store.get_observation(id).await {
                    Ok(Some(record)) if record.agent == ac.id => {
            ...
            let Some(integration) = args.integration.or_else(|| integration_of(&external_ref))
            else {
                return Ok(ToolOutput::error(
            ...
            Ok(
                match self
                    .store
                    .observations_by_ref(ac.id, &integration, &external_ref, limit)
                    .await
                {
...
mod tests {
    ...
    fn call(arguments: Value) -> ToolCall {
        ToolCall {
            call_id: "1".to_owned(),
            name: OBSERVATION.clone(),
            arguments,
        }
    }

    fn agent_ctx(agent: AgentId) -> AgentContext {
        AgentContext {
            id: agent,
            slug: "dev".to_owned(),
    ...
    async fn a_cited_observation_id_resolves_to_its_payload() {
        ...
        let ac = agent_ctx(agent);
        let ctx = ToolContext::agent(&sandbox, &token, &ac);
        let out = tool.call(&call(json!({ "id": id })), ctx).await.unwrap();
    ...
    async fn a_lookup_without_id_or_ref_explains_itself() {
        ...
        let ac = agent_ctx(agent);
        let ctx = ToolContext::agent(&sandbox, &token, &ac);
        let out = tool.call(&call(json!({})), ctx).await.unwrap();
        assert!(out.is_error);
    }
}
